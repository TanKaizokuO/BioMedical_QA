...
| **G2** — Joint Attribution Gate | **Sep 6, 2026** | **PARITY CLOSED & FIRST READ COMPUTED (2026-08-14)** | On dev, joint attribution beats post-hoc citation on citation-F1 by margin > paired-bootstrap CI; ≥95% valid claim parse. *(Parity loop terminated Aug 14 at 1/10 iter; first F1 unblinded read: Joint 0.264 vs Post-hoc 0.345, `docs/harvest/first_citation_f1.md`).* |
...
- ~~Compute initial citation-F1 baseline comparison (~Aug 31).~~ *(Computed Aug 14, 2026, 17 days early — `scripts/first_citation_f1.py` on `parity_iter1b` → Joint F1 0.264 vs Post-hoc F1 0.345, `docs/harvest/first_citation_f1.md`).*
